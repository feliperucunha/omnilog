 ...
 * sort value: relevance (no-op), title_asc, title_desc, year_desc, year_asc, score_desc, start_date_*.
 */
export function sortSearchResults(results: SearchResult[], sort: string | undefined): SearchResult[] {
  ...
  switch (sort) {
    ...
    case "score_desc":
      return arr.sort((a, b) => (scoreNum(b) ?? -Infinity) - (scoreNum(a) ?? -Infinity));
    case "year_desc":
